...
class SecureStorageService {
  ...
  static const _wakeTokenKey = 'wake_api_token';
  ...
  Future<String?> readWakeToken() => _storage.read(key: _wakeTokenKey);

  Future<void> writeWakeToken(String token) async {
    final trimmed = token.trim();
    if (trimmed.isEmpty) {
      await clearWakeToken();
      return;
    }
    await _storage.write(key: _wakeTokenKey, value: trimmed);
  }

  Future<void> clearWakeToken() => _storage.delete(key: _wakeTokenKey);
