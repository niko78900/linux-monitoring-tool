...
target_path=$config_dir/dashboard-control-read.env
...
if [ "$check_only" = true ]; then
  echo "dashboard bridge environment inputs validated"
  exit 0
fi
...
umask 077
temporary_path=$(mktemp "$config_dir/.dashboard-control-read.env.XXXXXX")
trap 'rm -f -- "$temporary_path"' 0 HUP INT TERM
token=$(openssl rand -hex 32)
{
  printf 'DASHBOARD_CONTROL_READ_TOKEN=%s\n' "$token"
  printf 'DASHBOARD_CONTROL_BRIDGE_HOST=%s\n' "$bridge_host"
  printf 'DASHBOARD_CONTROL_BRIDGE_PORT=4043\n'
  printf 'DASHBOARD_CONTROL_ALLOWED_NETWORKS=%s\n' "$allowed_network"
  printf 'KNOWN_DEVICES_CONFIG_PATH=/etc/linux-monitor/control-agent/known_devices.yaml\n'
  printf 'MANAGED_HOSTS_CONFIG_PATH=/etc/linux-monitor/control-agent/managed_hosts.yaml\n'
  printf 'SERVICES_CONFIG_PATH=/etc/linux-monitor/control-agent/services.yaml\n'
} > "$temporary_path"
token=
...
echo "installed protected bridge environment at $target_path"
